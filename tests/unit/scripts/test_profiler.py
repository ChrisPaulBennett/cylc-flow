...
def test_parse_memory_file(tmpdir):
    ...
    # Test the parse_memory_file function (values reported in megabytes)
    assert parse_memory_file(good_process_object_v1) == 2
    assert parse_memory_file(good_process_object_v2) == 3
...
def test_parse_memory_allocated(tmp_path_factory):
    ...
    assert parse_memory_allocated(good_process_object_v2) == 100
    with pytest.raises(CylcProfilerError) as excinfo:
    ...
    assert parse_memory_allocated(bad_process_object_v2_2) == 100
